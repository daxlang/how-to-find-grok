...
以 noise=20% 为例。噪声足够高时，只有精确的 wd 区间才能让模型泛化。

### Step 1：宽扫描定位窗口

![broad scan](figures/fig5_broad_scan.png?v=3)

从 wd=0.1 到 2.0，每个 wd 跑一遍。wd<1.5 全部不压缩（灰色）→ 排除。wd=1.8-2.0 开始出现压缩 → 候选区间。

### Step 2：窄扫描精确调参

![fine tune](figures/fig6_fine_tune.png?v=3)

在候选区间内（wd=2.0-2.2），每 0.05 一步精细扫描。弧底标注了 erank 反转后的最低点 → 最优泛化处的早停信号。

### 调参流程

1. 宽扫描 wd=[0.1, 0.3, 0.5, 1.0, 1.5, 2.0] → 找到出现压缩的区间
2. 在区间内以 0.05 步长窄扫描 → 找到最深弧的 wd
3. 弧底 epoch 作为早停点 → 无需验证集
